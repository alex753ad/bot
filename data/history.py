...
_NEW_COLUMNS = [
    ...
    # [NEW LOG] добавлено 18.07 — крупнейший датасет проекта, докидываем ключевые поля
    ("natr_1m", "REAL"),                 # NATR% 1m на момент касания (лучший разделитель по S2)
    ("natr_5m", "REAL"),
    ("natr_15m", "REAL"),
    ("candle_staleness_sec", "REAL"),    # возраст последней 1m-свечи (протухшие данные)
    ("confirm_delay_sec", "REAL"),       # задержка записи исхода от касания (аудит остаточной утечки)
    ("bot_version", "TEXT"),             # версия кода
...
async def save_level_outcome(
    ...
    natr_5m: float = None,
    natr_15m: float = None,
    candle_staleness_sec: float = None,
    confirm_delay_sec: float = None,
    bot_version: str = None,
) -> None:
    # [NEW LOG] NATR/staleness считаем здесь, если не переданы — не трогая hot-path monitor.
    if natr_1m is None:
        try:
            from data.collector import candles_1m as _c1, candles_5m as _c5, candles_15m as _c15
            import time as _t

            def _natr(cs, p=14):
                if not cs or len(cs) < p + 1:
                    return None
                w = cs[-(p + 1):]
                trs = [max(w[k]["high"] - w[k]["low"],
                           abs(w[k]["high"] - w[k - 1]["close"]),
                           abs(w[k]["low"] - w[k - 1]["close"])) for k in range(1, len(w))]
                cur = w[-1]["close"]
                return round(sum(trs) / len(trs) / cur * 100, 3) if cur > 0 else None
            natr_1m = _natr(_c1.get(symbol, []))
            natr_5m = _natr(_c5.get(symbol, []))
            natr_15m = _natr(_c15.get(symbol, []))
            c1 = _c1.get(symbol, [])
            if c1:
                candle_staleness_sec = round(_t.time() - c1[-1]["open_time"] / 1000.0, 1)
        except Exception:
            pass
    if bot_version is None:
        try:
            from version import BOT_VERSION as _bv
            bot_version = _bv
        except Exception:
            pass
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute(
                ...
                (symbol, level, level_type, strength_claude, approach_type,
                 ...
                 natr_1m, natr_5m, natr_15m, candle_staleness_sec, confirm_delay_sec, bot_version)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
