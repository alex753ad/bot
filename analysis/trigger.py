...
def _count_approaches(symbol: str, level: float, atr: float,
                       ...
                       end_time: float = None) -> tuple[int, set]:
    ...
    end_time: [FIX HIST-LEAK] верхняя граница (unix-секунды). Если задана, свечи ПОСЛЕ
    неё не учитываются. Нужна для честной записи touches в history.db: счётчик должен
    отражать состояние НА МОМЕНТ КАСАНИЯ, а не на момент подтверждения исхода — иначе
    у пробоев (подтверждаются позже) touches успевает вырасти → утечка разметки, из-за
    которой touches>=2 в датасете имел 0% отбоя и был непригоден для обучения.
    ...
    for c in c1m:
        if pump_high_time and c["open_time"] < pump_high_time:
            was_near = False
            continue
        # [FIX HIST-LEAK] не считать свечи после end_time (момента касания)
        if end_time is not None and c["open_time"] > end_time * 1000:
            break
