...
def _real_touches(symbol: str, level: float, end_time: float = None) -> int:
    ...
    try:
        ...
        if _atr and _atr > 0:
            ...
            # [FIX HIST-LEAK] end_time = момент касания: без него touches считался
            # в момент подтверждения исхода — у пробоев (подтверждаются позже)
            # счётчик успевал вырасти → утечка разметки (touches>=2 → 0% отбоя).
            return _count_approaches(symbol, level, _atr,
                                     anchor_time=_origin_anchor(symbol, level),
                                     end_time=end_time)[0]
    except Exception:
        pass
    return 1
...
async def _log_bounce_outcome(
    ...
    await save_level_outcome(
        ...
        touches=_real_touches(symbol, level, end_time=touch_start_time),  # [FIX HIST-LEAK]
        result="отбой",
