...
class Strategy5Live(Strategy5Continuation):
    ...
    async def _finalize_closed(self, t: dict, reason: str) -> None:
        ...
        exec_err = None
        execs = []
        # [FIX EXEC-RACE] Ретрай до появления Sell-ноги.
        # Диагностика 18-22.07 показала: reconcile видит position size=0 и сразу
        # запрашивает executions, но биржа записывает исполнение SL на 1-3 сек позже
        # → в ответе только ['Buy'] → PnL считался оценочно (qty_mismatch 17/19).
        # Ждём, пока появится хотя бы одна Sell, максимум ~9 сек.
        for attempt in range(5):
            try:
                execs = await api.get_executions_by_symbol(symbol, start_ms=start_ms, limit=100)
            except Exception as e:
                execs = []; exec_err = str(e)
            if any(e.get("side") == "Sell" for e in execs):
                break
            if attempt < 4:
                await asyncio.sleep(1.5 + attempt)  # 1.5, 2.5, 3.5, 4.5 сек
        # [NEW LOG] сырой диагностический слепок ответа executions
        logger.info(
            "S5Live executions raw", trade_id=trade_id, symbol=symbol,
            exec_count=len(execs), exec_err=exec_err, attempts=attempt + 1,
            start_ms=start_ms,
            sides=[e.get("side") for e in execs][:8],
            qtys=[e.get("execQty") for e in execs][:8],
        )
        ...
        if buy_qty <= 0 or sell_qty <= 0:
            ...
            logger.warning("S5Live: no executions, PnL estimated", trade_id=trade_id,
                           exec_count=len(execs), exec_err=exec_err,
