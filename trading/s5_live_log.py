...
def _ensure(con: sqlite3.Connection) -> None:
    ...
    con.commit()
    # [FIX] авто-миграция недостающих столбцов
    _need = {"bot_version": "TEXT"}
    _exist = {r[1] for r in con.execute("PRAGMA table_info(s5_live_trades)")}
    for _n, _t in _need.items():
        if _n not in _exist:
            try: con.execute(f"ALTER TABLE s5_live_trades ADD COLUMN {_n} {_t}")
    ...
    try:
        ...
        con.execute("""
            ...
            (trade_id, symbol, status, entry_time, entry_dt, entry_price, qty,
             ...
             entry_order_id, sl_order_id, bot_version, basis, events_json)
            VALUES (?,?,'open',?,?,?,?,?,?,?,?,?,?,?,?,?, '[]')""", (
            ...
            t.get("entry_order_id"), t.get("sl_order_id"), BOT_VERSION, t.get("basis"),
        ))
        con.commit()
    finally:
        con.close()
