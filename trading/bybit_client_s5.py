...
async def get_executions_by_symbol(symbol: str, start_ms: Optional[int] = None,
    ...
    if resp.get("retCode") != 0:
        # [NEW LOG] не глотать ошибку — вернуть диагностику наверх через лог
        logger.warning("S5 get_executions non-zero retCode", symbol=symbol,
                       retCode=resp.get("retCode"), retMsg=resp.get("retMsg"))
        return []
