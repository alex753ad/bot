...
class Strategy5Continuation(BaseStrategy):
    ...
    def __init__(self) -> None:
        ...
        # [NEW LOG] история для контекстных полей
        self._fire_history: dict[str, list] = {}   # symbol → [ts, ts, ...] за 24ч
        self._last_sl_ts: dict[str, float] = {}     # symbol → ts последнего SL
        self._last_loss_ts: dict[str, float] = {}   # symbol → ts последней УБЫТОЧНОЙ сделки
        self._mfe_mae: dict[str, list] = {}         # trade_id → [mfe_pct, mae_pct]
    ...
    def _detect_signal(self, symbol: str) -> Optional[dict]:
        ...
        try:
            ...
            if delta is None:
                ...
                if bv + sv > 0:
                    ...
                    sell_v = round(sv, 4)
                    delta = round(bv - sv, 4)
                    delta_src = "candle_proxy"
        except Exception:
            pass

        # [NEW LOG] контекстные поля для калибровки, только наблюдение
        # Свежесть последней 1m-свечи (протухшие данные → мусорный вход)
        staleness = None
        try:
            staleness = round(time.time() - c1m[-1]["open_time"] / 1000.0, 1)
        except Exception:
            pass
        # Индекс входа по символу за 24ч и время с последнего SL
        now = time.time()
        hist = [t for t in self._fire_history.get(symbol, []) if now - t < 86400]
        series_idx = len(hist) + 1
        last_sl = self._last_sl_ts.get(symbol)
        sec_since_sl = round(now - last_sl, 1) if last_sl else None
        # Спред стакана (если L2 доступен через тот же снапшот, что у S2)
        # [FIX SPREAD] прошлая версия импортировала несуществующий
        # data.collector.get_orderbook_snapshot → поле было пустым (0/19).
        # Прокси по свечам: средний (high-low)/close последних 3×1m — грубая мера
        # «разъезда» цены. Настоящий L2-спред добавим, когда будет функция в клиенте.
        spread_pct = None
        try:
            last3 = c1m[-3:]
            vals = [(c["high"] - c["low"]) / c["close"] * 100 for c in last3 if c["close"] > 0]
            if vals:
                spread_pct = round(sum(vals) / len(vals), 4)
        except Exception:
            pass
        ...
        return {
            ...
            "candle_staleness_sec": staleness,
            "series_index": series_idx,
            "sec_since_last_sl": sec_since_sl,
            "basis": basis,
        }

    # [УДАЛЁН ФАЗОВЫЙ ГЕЙТ] За 18-22.07 market_phase = 'unknown' во ВСЕХ 19 сигналах:
    # detect_market_phase не классифицирует свежие пампы, которые ловит S5. Гейт
    # bleed/drop_tradeable не срабатывал ни разу — мёртвый код, дающий ложное чувство
    # защиты. Выигрыш +7.5$, показанный ранее на 33 сделках, был на старых данных.
    # Фаза продолжает ЛОГИРОВАТЬСЯ в основании сигнала (для анализа), но не гейтит.
    S5_BLOCKED_PHASES: set = set()

    # [FIX RE-ENTRY] Не входить в символ N часов после УБЫТОЧНОЙ сделки по нему.
    # Кулдаун по времени (2ч) не помог: BANKUSDT дал 4 входа за сутки подряд
    # (series_index 1→4) на суммарные -7.02$, интервалы были >2ч. Решает не время,
    # а исход: если сетап на символе только что не сработал, контекст не изменился.
    S5_LOSS_BLOCK_HOURS = 12.0
    ...
    async def _scan_symbol(self, symbol: str) -> None:
        ...
        # [FIX RE-ENTRY] блок после недавнего убытка по этому символу
        last_loss = self._last_loss_ts.get(symbol)
        if last_loss and (time.time() - last_loss) < self.S5_LOSS_BLOCK_HOURS * 3600:
            logger.debug("S5 skip: recent loss on symbol", symbol=symbol,
                         hours_since_loss=round((time.time() - last_loss) / 3600, 1))
            return
        ...
        # [NEW LOG] история входов по символу (для series_index следующих сигналов)
        self._fire_history.setdefault(symbol, []).append(time.time())
    ...
    async def _check_exit(self, trade: dict, current_price: float) -> None:
        ...
        # [NEW LOG] MFE/MAE — максимальный ход в плюс/минус за жизнь сделки.
        # Отвечает на вопрос из спора о выходах: «выбило по BE, а дошла ли до 1.5R?»
        if entry_price > 0:
            fav = (current_price - entry_price) / entry_price * 100
            adv = (entry_price - current_price) / entry_price * 100
            st = self._mfe_mae.setdefault(trade_id, [0.0, 0.0])
            st[0] = max(st[0], fav)
            st[1] = max(st[1], adv)
    ...
    async def _send_close_message(self, trade: dict, exit_price: float, reason: str) -> None:
        ...
        tid = trade["trade_id"]
        mfe, mae = self._mfe_mae.pop(tid, [None, None])
        await s5_log.close_trade(tid, exit_price, reason, tp1_hit, pnl_pct, pnl_usdt,
                                 mfe_pct=mfe, mae_pct=mae)
        # [NEW LOG] запомнить время SL по символу (для sec_since_last_sl следующих сигналов)
        if reason in ("stop_loss", "sl_exchange"):
            self._last_sl_ts[trade["symbol"]] = time.time()
        # [FIX RE-ENTRY] любая убыточная сделка блокирует символ на S5_LOSS_BLOCK_HOURS
        if pnl_pct < 0:
            self._last_loss_ts[trade["symbol"]] = time.time()
        # [NEW LOG] пост-выход трекер: куда ушла цена 30 мин (ловит «продал дно»)
        try:
            asyncio.create_task(self._track_post_exit(tid, trade["symbol"]))
        except Exception:
            pass
        logger.info(
            "S5 trade closed", symbol=trade["symbol"], reason=reason,
            pnl_pct=round(pnl_pct, 2), pnl_usdt=round(pnl_usdt, 3),
            tp1_hit=tp1_hit, mfe_pct=round(mfe, 2) if mfe else None,
            mae_pct=round(mae, 2) if mae else None, trade_id=tid,
        )

    async def _track_post_exit(self, trade_id: str, symbol: str) -> None:
        """30 мин после выхода следим за экстремумами цены — записываем в s5-БД."""
        hi = lo = None
        for _ in range(30):
        ...
        if hi is not None:
            await s5_log.set_post_exit(trade_id, round(hi, 10), round(lo, 10))
