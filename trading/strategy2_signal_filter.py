...
def _natr_pct(candles: list, period: int = 14):
    """NATR% = средний True Range за period свечей / текущая цена * 100. None при нехватке."""
...
try:
    from version import BOT_VERSION as _BOT_VERSION
except Exception:
    _BOT_VERSION = "unknown"


def _candle_staleness(symbol: str):
    """Возраст последней 1m-свечи, сек. Высокое значение = вход по протухшим данным."""
    import time as _t
    c = candles_1m.get(symbol, [])
    if not c:
        return None
    try:
        return round(_t.time() - c[-1]["open_time"] / 1000.0, 1)
    except Exception:
        return None
...
class Strategy2SignalFilter:
    ...
    async def check(
        ...
        logger.info(
            ...
            natr_1m=_live.get("natr_1m"),
            natr_5m=_live.get("natr_5m"),
            natr_15m=_live.get("natr_15m"),
            candle_staleness_sec=_candle_staleness(symbol),
